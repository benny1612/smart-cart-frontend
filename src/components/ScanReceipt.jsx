        ...
        <div className="space-y-6 animate-in fade-in duration-500">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 sm:p-6 bg-gray-50 rounded-2xl border border-gray-200">
            <div>
              ...
              <input 
                className="w-full p-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                value={result.storeName || ""}
                onChange={(e) => handleStoreChange('storeName', e.target.value)}
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1">סניף / עיר</label>
              <input 
                className="w-full p-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                value={result.branchName || ""}
                onChange={(e) => handleStoreChange('branchName', e.target.value)}
              />
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-gray-700 flex justify-between items-center px-1">
              רשימת מוצרים
              <button onClick={addItem} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700">+ הוסף</button>
            </h3>
              ...
              <div key={idx} className="flex flex-col sm:flex-row gap-2 sm:gap-3 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm relative group">
                ...
                <div className="flex items-center gap-2 justify-between sm:justify-end">
                  <div className="relative">
                    <input 
                      type="number"
                      className="w-24 sm:w-28 p-2 bg-gray-100 rounded-lg outline-none text-left font-bold text-blue-700 text-sm"
                      value={item.price}
